...
if(isset($_POST['recover'])){
	...
	if(!$foundEmail){
		$recoverDomiciliario = new Domiciliario();
		if($recoverDomiciliario -> existEmail($_POST['email'])) {;
			$generatedPassword = rand(100000,999999);
			$recoverDomiciliario -> recoverPassword($_POST['email'], $generatedPassword);
		$foundEmail = true;
		}
	}
	if(!$foundEmail){
		$recoverCliente = new Cliente();
		if($recoverCliente -> existEmail($_POST['email'])) {;
			$generatedPassword = rand(100000,999999);
			$recoverCliente -> recoverPassword($_POST['email'], $generatedPassword);
		$foundEmail = true;
		}
	}
	if(!$foundEmail){
		$recoverCajero = new Cajero();
		if($recoverCajero -> existEmail($_POST['email'])) {;
			$generatedPassword = rand(100000,999999);
			$recoverCajero -> recoverPassword($_POST['email'], $generatedPassword);
		$foundEmail = true;
		}
	}
	if($foundEmail){
		$to=$_POST['email'];
		$subject="Recuperación de clave para PAR";
		$from="FROM: PAR <contact@itiud.org>";
